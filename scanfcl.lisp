...
(define-compiler-macro sscanf (&whole form input-string control-string)
...
(defun make-char-matcher (char)
  `(lambda ()
     (if (looking-at ,char)
         (forward)
         (warn "Couldn't match char '~C' at '~D'" ,char *index*))))
...
(defun make-integer-scanner (suppressp field-width length-modifier &key radix)
  `(lambda ()
     ...
     (let ((sign 1)
           (result 0)
           (limit ,(if field-width
                       `(min (+ *index* ,field-width) *limit*)
                       '*limit*)))
       (cond ((looking-at #\+)
              (forward))
             ((looking-at #\-)
       ...
       (if (digit-char-p (current) ,radix)
           (loop for char = (current)
                 until (or (null char) (not (digit-char-p char ,radix)) (>= *index* limit))
                 do (setf result (+ (* result ,radix) (digit-char-p char ,radix)))
                    (forward)
                 finally ,(if suppressp '(values) '(push (* sign result) *result*)))
           (warn "Can't match %d at index ~D, got '~A'" *index* (current))))))
...
(defun make-signed-integer-scanner (suppressp field-width length-modifier)
  `(lambda ()
     ...
     (let ((sign 1)
       ...
       (warn "Can't match %i at index ~D, got '~A'" *index* (current)))))
     ...
     (let ((sign 1d0)
       ...
       (cond ((or (looking-at #\I)
              ...
              (loop for char = (current)
                    ...
                    until (or (null char) (>= *index* limit)
                    ...
                    do (forward)
                                ...
                                (error "Can't match (inf) float at index ~D, got '~A'" *index* (current)))))
             ((or (looking-at #\N)
                  (looking-at #\n))
              (loop for char = (current)
                    ...
                    do (forward)
                                ...
                                (progn
                                  (when (looking-at #\()
                                    (forward)
                                    (loop for char = (current)
                                          while (isnanbody char)
                                          do (forward))
                                    (if (looking-at #\) )
                                        (forward)
                                        (error "Can't match (nan) float at index ~D, got '~A'" *index* (current))))
  ...
  `(lambda ()
     ...
     (if (not (isspace (current)))
         (let ((limit ,(if field-width
               ...
               (result '()))
           (loop for char = (current)
                 until (or (null char) (isspace char) (>= *index* limit))
                 do (push char result)
                    (forward)
                 finally ,(if suppressp
                              '(values)
                              '(push (coerce (reverse result) 'string) *result*))))
         (warn "Can't match %s at index ~D, got '~A'" *index* (current)))))
